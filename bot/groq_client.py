            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    if attempt < self.max_retries - 1:
                        print(
                            f"  ⏱️ Rate limited, waiting {retry_after}s...", flush=True
                        )
                    ...
                    notify_admin(
                        "Groq", "❌ Auth failed", reason, {"model": self.model_name}
                    )
                    raise GroqError(reason)
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        continue
                    notify_admin(
                        "Groq", f"❌ Server error", reason, {"model": self.model_name}
                    )
                    raise GroqError(reason)
                ...
                try:
                    ...
                    if attempt < self.max_retries - 1:
                        print(
                            f"  ✗ Validation failed, retrying... ({str(e)[:100]})",
                            flush=True,
                        )
                ...
                if attempt < self.max_retries - 1:
                    ...
                    continue
                notify_admin(
                    "Groq", "❌ Network error", reason, {"model": self.model_name}
                )
                raise GroqError(reason)
